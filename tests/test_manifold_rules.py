...
from geomech.core.operations.geometry import Cross, Dot, Hat
from geomech.core.operations.multiplication import MMMul, MVMul, SVMul
...
class TestS2ManifoldRules:
    ...
    def test_variation_of_omega_expands(self):
        ...
        assert result.nodes[1].right == Scalar("(-1)", value=-1, attr=["Constant"])
...
class TestMixedManifoldRules:
    ...
    def test_s2_and_plain_vector(self):
        ...
        has_variation_x = any(
            isinstance(n, Variation) and str(n.expr) == "x" for n in result.nodes
        )
